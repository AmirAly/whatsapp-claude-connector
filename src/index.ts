  ...
  tools: [
    {
      ...
      description:
        'Connect to WhatsApp. On first run returns a QR code to scan with your phone. ' +
        'On subsequent runs restores the saved session automatically.',
      inputSchema: { type: 'object', properties: {}, required: [] },
    },
    {
      name: 'whatsapp_status',
      description: 'Check if WhatsApp is connected and which phone number is active.',
      inputSchema: { type: 'object', properties: {}, required: [] },
    },
    {
      name: 'whatsapp_get_chats',
      description: 'List recent WhatsApp chats sorted by latest message.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Search for a chat by contact name, group name, or phone number.',
      inputSchema: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'Name, group name, or phone number to search for' },
        },
        required: ['query'],
      },
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          chatId: {
            type: 'string',
            description: 'Chat ID (e.g. 201012345678@s.whatsapp.net) or plain phone number',
          },
  ...
  try {
    ...
    switch (name) {
      ...
      case 'whatsapp_get_chats':
        result = toolGetChats(args as { limit?: number });
        break;

      case 'whatsapp_search_chat':
        result = toolSearchChat(args as { query: string });
        break;

      case 'whatsapp_get_messages':
        result = toolGetMessages(args as { chatId: string; limit?: number });
        break;

      case 'whatsapp_send_message':
        result = await toolSendMessage(args as { chatId: string; text: string });
        break;
